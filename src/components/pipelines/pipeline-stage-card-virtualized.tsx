...
export const PipelineStageCardVirtualized = memo(function PipelineStageCardVirtualized({
  stage,
  isSelected,
  selectedContacts,
  searchQuery,
  ...
  // Virtual scrolling for contacts
  const rowVirtualizer = useVirtualizer({
    count: stage.contacts.length,
    getScrollElement: () => parentRef.current,
    estimateSize: () => 80, // Estimated height of each contact card
  ...
  return (
    <div
      ref={setNodeRef}
      className={`shrink-0 w-80 transition-colors ${
        isOver ? 'opacity-50' : ''
      }`}
    >
      <Card className={`h-full ${isSelected ? 'ring-2 ring-primary' : ''}`}>
        ...
        <CardContent className="p-0">
          {stage.contacts.length === 0 ? (
            ...
            <div
              ref={parentRef}
              className="max-h-[600px] overflow-y-auto px-4 pb-4"
            >
              <div
                style={{
              ...
              >
                {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                  const contact = stage.contacts[virtualRow.index];
                  return (
                    <div
                      key={contact.id}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2 px-4 pb-4 border-t">
              <Button
                variant="ghost"
                size="sm"
